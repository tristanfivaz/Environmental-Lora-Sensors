...
float phValue1, phValue2, phValue3, phValueAvg;
...
// Schedule TX every 3 minutes (might become longer due to duty
// cycle limitations).
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    voltage1 = analogRead(PH_PIN) / 1024.0 * 5000; // read the voltage
    ...
    phValue2 = ph.readPH(voltage2, temperature); // convert voltage to pH with temperature compensation
    delay(800);

    voltage3 = analogRead(PH_PIN) / 1024.0 * 5000; // read the voltage
    phValue3 = ph.readPH(voltage3, temperature); // convert voltage to pH with temperature compensation
    delay(800);

    phValueAvg = (phValue1 + phValue2 + phValue3) / 3;

    turbreading = analogRead(A0);
    tvoltage = turbreading * (5.0 / 1024.0);
    ...
    int phValueInt = phValueAvg * 100;
    int turbidityInt = tvoltage * 100;

    //Break the humidity, temperature and heat index into Bytes in individual buffer arrays
    byte payloadA[2];
    payloadA[0] = highByte(temperatureInt);
    payloadA[1] = lowByte(temperatureInt);
    byte payloadB[2];
    payloadB[0] = highByte(phValueInt);
    payloadB[1] = lowByte(phValueInt);
    byte payloadC[2];
    payloadC[0] = highByte(turbidityInt);
    payloadC[1] = lowByte(turbidityInt);
